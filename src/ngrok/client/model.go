...
type ClientModel struct {
	...
	id      string
	tunnels map[string]mvc.Tunnel //隧道map

	serverVersion string //服务器版本
	//metrics       *ClientMetrics
	updateStatus mvc.UpdateStatus                //升级状态
	connStatus   mvc.ConnStatus                  //连接状态
	protoMap     map[string]proto.Protocol       //协议map
	protocols    []proto.Protocol                //协议数组
	ctl          mvc.Controller                  //控制器
	serverAddr   string                          //服务器地址
	proxyUrl     string                          //代理路径
	authToken    string                          //权限
	tlsConfig    *tls.Config                     //tsl配置
	tunnelConfig map[string]*TunnelConfiguration //隧道配置
...
func newClientModel(config *Configuration, ctl mvc.Controller) *ClientModel {
	...
	m := &ClientModel{
		...
		// 链接状态，0=ConnConnecting，1=ConnReconnecting，2=ConnOnline
		connStatus: mvc.ConnConnecting,

		// 升级状态，0=UpdateNone，-1=UpdateInstalling，-2=UpdateReady，-3=UpdateAvailable
...
%s`, len(badGatewayBody), badGatewayBody)))
	...
	//local := tunnel.Protocol.WrapConn(localConn, mvc.ConnectionContext{Tunnel: tunnel, ClientAddr: startPxy.ClientAddr})
